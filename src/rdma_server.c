...
static int send_server_metadata_to_client() 
{
    ...
    info("Client side buffer information is received...\n");
    show_rdma_buffer_attr(&client_metadata_attr);
    info("The client has requested buffer length of : %u bytes \n", 
...
static int disconnect_and_cleanup()
{
    ...
    info("A disconnect event is received from the client...\n");
...
int main(int argc, char **argv) 
{
    ...
    while(1) {
        struct rdma_cm_event *cm_event = NULL;
        const int recv_ret = receive_rdma_cm_event(cm_event_channel, &cm_event);
        if (recv_ret) {
            rdma_error("Failed to receive cm event, ret = %d \n", recv_ret);
            return recv_ret;
        }
        if (cm_event->event == RDMA_CM_EVENT_DISCONNECTED) {            
            ...
            rdma_error("Received a new connection request while already connected \n");
            return -1;
        } else {
            rdma_error("Received an unexpected event %d \n", cm_event->event);
            return -1;
        }
    }
	return 0;
